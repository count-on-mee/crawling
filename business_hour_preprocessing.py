...
try:
    ...
    #Time형식 변환
    new_df['open_time'] = pd.to_datetime(new_df['open_time'], format='%H:%M', errors='coerce').apply(lambda x: None if pd.isna(x) else x.time())
    new_df['close_time'] = pd.to_datetime(new_df['close_time'], format='%H:%M', errors='coerce').apply(lambda x: None if pd.isna(x) else x.time())
    new_df['break_start_time'] = pd.to_datetime(new_df['break_start_time'], format='%H:%M', errors='coerce').apply(lambda x: None if pd.isna(x) else x.time())
    new_df['break_end_time'] = pd.to_datetime(new_df['break_end_time'], format='%H:%M', errors='coerce').apply(lambda x: None if pd.isna(x) else x.time())
    new_df['last_order'] = pd.to_datetime(new_df['last_order'], format='%H:%M', errors='coerce').apply(lambda x: None if pd.isna(x) else x.time())
    ...
    for spot_id in spot_ids_with_more_than_7:
        ...
        for week in spot_data['week'].unique():
            ...
            lastorder_value = week_data['last_order'].dropna().iloc[0] if 'last_order' in week_data and not week_data['last_order'].dropna().empty else None
    ...
    for _, row in new_df.iterrows():
        ...
        (spot_business_hour_id, spot_business_hour_info_id, week, open_time, close_time,last_order, break_start_time, break_end_time)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
        """
        cursor.execute(sql, tuple(row))

    # spot_business_hour_info 테이블에 df 저장
    # spot_business_hour_info 테이블에 info 수정
    for _, row in df.iterrows():
        sql = """
        UPDATE spot_business_hour_info
        SET info = %s
        WHERE spot_business_hour_info_id = %s
        """
        cursor.execute(sql, (row['info'], row['spot_business_hour_info_id']))
